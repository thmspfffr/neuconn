...
for isubj = 1 : 34
  ...
  val(isubj,ifoi,1) = sum(sum(mask1(:,isubj)));
  val(isubj,ifoi,2) = sum(sum(mask2(:,isubj)));
...
plot(-log10(p(:,:,1).*23),'linewidth',3)
plot(-log10(p(:,:,2).*23),'linewidth',3)

line([1 23],[2 2],'linestyle','--','color','k','linewidth',3)


set(gca,'TickDir','out','XTick',[3 7 11 15 19 23],'XTickLabel',[4 8 16 32 64 128])
