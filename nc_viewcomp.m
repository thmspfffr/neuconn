...
while total_err == 0 
  ...
  for is = 1 : 20
    cnt = 0;
    for ibl = 1 : 2; for ifr = 1 : 2; for ise = 1 : 2
      cnt = cnt + 1;
  ...
  if split == 1
    ...
    seg1 = uicontrol('Units','normalized','Position',[0.35 0.3 0.3 0.2],'Style','pushbutton','String','Low Frequencies','Callback','which_freq = 1; close');
  end
  ...
  if exist('comp_low','var')
    comp_low.topo = W1;
    comp_low.unmixing = W1;
    comp_low.topolabel  = data_low.label;
    comp_low.grad =data_low.grad;
    comp_low.fsample = data_low.fsample;
    comp_low.time = data_low.time;
  end
