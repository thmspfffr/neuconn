...
%               
            ...
            if ~strcmp(filt,'pconn_beamformer') && length(SEGLENG)>1
%                             [F1,p] = getdipdir(cs(;,A); 

              [F1,p] = getdipdir(cs(:,:,foi),A); 
            elseif ~strcmp(filt,'pconn_beamformer') && length(SEGLENG)==1
              [F1,p] = getdipdir(cs(:,:,foi),A); 
            else 
              F1 = A';
            end   
            ...
            save([outdir sprintf('nc_powcorr_s%d_c%d_b%d_f%d_v%d.mat',isubj,icond,iblock,ifoi,v_out)],'resout','-v7.3');
            clear resout F1 p
            % -----------------------------------------------------------------
          else
            continue
