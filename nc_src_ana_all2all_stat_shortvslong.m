...
for ifoi = 1 : 23
  ...
  %
  if ~exist([outdir sprintf('nc_src_ana_all2all_stat_shortvslong_f%d_v%d_processing.txt',ifoi,v_out)])
    system(['touch ' outdir sprintf('nc_src_ana_all2all_stat_shortvslong_f%d_v%d_processing.txt',ifoi,v_out)]);
  else
    continue
  end
...
%   load  /home/tpfeffer/pconn/matlab/aalmask_grid_coarse
  ...
  allpow_lr = allpow(1:ceil(2113/2)+101,floor(2113/2-100):2113,:,1:22); clear allpow
  ...
  if perm
    ...
    save([outdir sprintf('nc_src_ana_all2all_stat_shortvslong_f%d_v%d.mat',ifoi,perm_v)],'cnt','cnt_perm','-v7.3');
...
% s = squeeze(allpow(:,1,1,1));
% 
% 
% showmri_transp_v2(mri,para,[grid s2])
% 
% 
