%% COUNTS THE NUMBER OF ALTERED LONG RANGE CONNECTIONS

% Computes connectivity strength across modules of the whole brain.
% Each region is inferred from the AAL atlas and long-range connectivity
% is defined as connections between AAL regions but not within.
...
% execute as
% --------------------------------------------------------
% nc_src_ana_all2all_stat_long_aal
% --------------------------------------------------------
% function nc_src_ana_all2all_stat_long_aal(rand_num)
...
%%
if ~exist([outdir sprintf('nc_src_ana_all2all_stat_long_aal_fc_v%d.mat',v_out)])
  allcorr = nan(91,91,2,NSUBJ,23);
  for ifoi = 1 : 23
    ...
    for reg1 = 1 : 91
      ...
      for reg2 = 1 : 91
        ...
        allcorr(reg1,reg2,:,:,ifoi) = squeeze(nanmean(tmp,1));
  ...
  save([outdir sprintf('nc_src_ana_all2all_stat_long_aal_fc_v%d.mat',v_out)],'allcorr','-v7.3');
...
for ifoi = 1  : 23
   %
    if ~exist([outdir sprintf('nc_src_ana_all2all_stat_long_aal_f%d_v%d_processing.txt',ifoi,v_out)])
      system(['touch ' outdir sprintf('nc_src_ana_all2all_stat_long_aal_f%d_v%d_processing.txt',ifoi,v_out)]);
    else
      continue
    end
    
  a     = jh_ranksum(cat(3,squeeze(allcorr(:,:,1,:,ifoi)),squeeze(allcorr(:,:,2,:,ifoi))));
  ...
  clear fc x_ms m_hc s_hc tmp allpow
  %%
  % ---------------------------------------------------
  % PERMUTATION TEST
  % ---------------------------------------------------
  ...
  allpow	= squeeze(cat(4,allcorr(:,:,1,:,ifoi),allcorr(:,:,2,:,ifoi)));
  tic
  ...
  for iperm = 1 : NPERM
    ...
    a     = jh_ranksum(cat(3,squeeze(allpow_perm(:,:,1,:)),squeeze(allpow_perm(:,:,2,:))));
    b     = a(logical(triu(ones(size(a)),1))); clear a
    pn    = [b>0 b<0];
    p     = 2*normcdf(-abs(b));
    cnt_perm(:,iperm) 	= [sum(p<thresh) sum(p<thresh)/length(p) sum(p(pn(:,1))<thresh) sum(p(pn(:,2))<thresh)]; clear a
  ...
  save([outdir sprintf('nc_src_ana_all2all_stat_long_aal_f%d_v%d.mat',ifoi,v_out)],'cnt','cnt_perm','-v7.3');
...
for ifoi = 1 : 2 : NFREQ*2
  it = it + 1;
  load([outdir sprintf('nc_src_ana_all2all_stat_long_aal_f%d_v%d.mat',it,v_out)]);
  
  
  all_cnt(:,ifoi:ifoi+1)  = cnt_perm(3:4,:)';
  acnt(:,ifoi:ifoi+1)     = cnt(end,3:4);
end
...
ylabel('P-VALUE')
set(gca,'YLim',[0 2.5]);
saveas(gcf,sprintf([plotdir 'nc_src_all2all_stats_long_aal_corr_v%d.fig'],v_out),'fig')
...
saveas(gcf,sprintf([plotdir 'nc_src_all2all_stats_long_aal_uncorr_v%d.fig'],v_out),'fig')
